class ServerFactory
  def self.create_server(params, api_key)
    ...
    new_server.update_attributes(provider_id: new_droplet.id, memory: new_droplet.memory, cpus: new_droplet.vcpus, disk: new_droplet.disk, status: new_droplet.status) if new_droplet
